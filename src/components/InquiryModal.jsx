...
function InquiryModal({open,onClose,onSubmit}) {
    ...
    if(!open) return null
    function handleBlur(f){ setTouched(t=>({...t,[f]:true})) }
    ...
    function resetAndClose(){ onClose(); setTimeout(()=>{ setFirstName(''); setLastName(''); setEmail(''); setPhone(''); setMessage(''); setTouched({}); setSubmitted(false); setSubmitting(false); setErrors({}) },250) }
    return (
